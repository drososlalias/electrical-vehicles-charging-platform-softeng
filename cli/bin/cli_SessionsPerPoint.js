...
// command initializer
function SessionsPerPoint (yargs) {
    argv = yargs
    .usage('Usage: $0 SessionsPerPoint --point <pointId> --datefrom <date> --dateto <date> --format [json | csv] --apikey <API key>\n')
    .option("point",    { describe: "Point Id",                             type: "string", demandOption: true })
    ...
    const nonEmptyInput = (argv.point != "") && (argv.datefrom != "") && (argv.dateto != "") && (aux.checkFormat(argv.format)) && (argv.apikey != "");
    if (!nonEmptyInput) { console.log('\x1b[31m%s\x1b[0m', 'Parameters must have a specific format'); yargs.showHelp() }
    else                { SessionsPerPoint_aux(argv.point, argv.datefrom, argv.dateto, argv.format, argv.apikey) }
}

// gets the job done
function SessionsPerPoint_aux (pointId, datefrom, dateto, format, apikey) {

    const url = "https://localhost:8765/evcharge/api/SessionsPerPoint/" + pointId + "/" + datefrom + "/" + dateto + "?format=" + format;
