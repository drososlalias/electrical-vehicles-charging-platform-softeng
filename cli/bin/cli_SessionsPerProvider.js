...
// command initializer
function SessionsPerProvider (yargs) {
    argv = yargs
    .usage('Usage: $0 SessionsPerProvider --provider <providerId> --datefrom <date> --dateto <date> --format [json | csv] --apikey <API key>\n')
    .option("provider", { describe: "Provider Id",                          type: "string", demandOption: true })
    ...
    const nonEmptyInput = (argv.provider != "") && (argv.datefrom != "") && (argv.dateto != "") && (aux.checkFormat(argv.format)) && (argv.apikey != "");
    if (!nonEmptyInput) { console.log('\x1b[31m%s\x1b[0m', 'Parameters must have a specific format'); yargs.showHelp() }
    else                { SessionsPerProvider_aux(argv.provider, argv.datefrom, argv.dateto, argv.format, argv.apikey) }
}

// gets the job done
function SessionsPerProvider_aux (providerId, datefrom, dateto, format, apikey) {

    const url = "https://localhost:8765/evcharge/api/SessionsPerStation/" + providerId + "/" + datefrom + "/" + dateto + "?format=" + format;
